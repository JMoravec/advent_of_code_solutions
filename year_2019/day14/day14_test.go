...
func TestCalculateTotalFuelForOreAmount(t *testing.T) {
	var tests = []struct {
		inputReactions []string
		expectedFuel   int
	}{
	...
	for _, test := range tests {
		testname := fmt.Sprintf("%v,%d", test.inputReactions, test.expectedFuel)
		t.Run(testname, func(t *testing.T) {
			reactions := createReactionHash(test.inputReactions)

			actualFuelReturned := calculateTotalFuelForOreAmount(1000000000000, reactions)
			assert.Equal(t, test.expectedFuel, actualFuelReturned)
		})
	}
}
