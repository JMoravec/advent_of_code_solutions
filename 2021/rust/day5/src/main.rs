...
impl Vent {
    ...
    fn create_lines(&self, use_diag: bool) -> Vec<Point> {
        ...
        if self.x1 == self.x2 {
            let (start, end) = if self.y1 > self.y2 {(self.y2, self.y1)} else {(self.y1, self.y2)};
            for i in start..(end+1) {
                points.push(Point::new(self.x1, i));
            }
        } else if self.y1 == self.y2 {
            let (start, end) = if self.x1 > self.x2 {(self.x2, self.x1)} else {(self.x1, self.x2)};
            for i in start..(end+1) {
            ...
            for i in 0..length {
                points.push(Point::new(self.x1+i, self.y1+i))
            }
        }
        points
    }
...
mod tests {
    ...
    fn test_vent_creation() {
        assert_eq!(
            Vent { x1: 0, y1: 9, x2: 5, y2: 9 },
            Vent::from_str("0,9 -> 5,9")
        );
        assert_eq!(
            Vent { x1: 8, y1: 0, x2: 0, y2: 8 },
            Vent::from_str("8,0 -> 0,8")
        );
        assert_eq!(
            Vent { x1: 9, y1: 4, x2: 3, y2: 4 },
            Vent::from_str("9,4 -> 3,4")
        );
        assert_eq!(
            Vent { x1: 2, y1: 2, x2: 2, y2: 1 },
            Vent::from_str("2,2 -> 2,1")
        );
        assert_eq!(
            Vent { x1: 7, y1: 0, x2: 7, y2: 4 },
            Vent::from_str("7,0 -> 7,4")
        );
        assert_eq!(
            Vent { x1: 6, y1: 4, x2: 2, y2: 0 },
            Vent::from_str("6,4 -> 2,0")
        );
        assert_eq!(
            Vent { x1: 0, y1: 9, x2: 2, y2: 9 },
            Vent::from_str("0,9 -> 2,9")
        );
        assert_eq!(
            Vent { x1: 3, y1: 4, x2: 1, y2: 4 },
            Vent::from_str("3,4 -> 1,4")
        );
        assert_eq!(
            Vent { x1: 0, y1: 0, x2: 8, y2: 8 },
            Vent::from_str("0,0 -> 8,8")
        );
        assert_eq!(
            Vent { x1: 5, y1: 5, x2: 8, y2: 2 },
